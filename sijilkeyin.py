...
if st.session_state.generated:
    ...
    for idx, row in enumerate(st.session_state.rows):
        ...
        with container:
            ...
            if cols[1].button("PNG", key=png_key):
                img_bytes = generate_certificate_png(name, ic)
                st.image(img_bytes, use_column_width=True)
            
            if cols[2].button("PDF", key=pdf_key):
                img_bytes = generate_certificate_png(name, ic)
...
# -------------------------
# Ayat Profesional + Link Projek
# -------------------------
st.markdown("""
<p style='text-align:center; font-size:16px; color:#0b5ed7; margin-top:20px;'>
Inovasi Sijil Automatik: Hasil ciptaan Narjihah binti Mohd Hashim, Mohamad Adham bin Abdul Malek, Nor Aqilah binti Aliasam, Muhammad Amin bin Muhammad Puzi, Siti Nur Amira binti Abdul Talib untuk mempercepatkan proses penerbitan sijil secara digital.
</p>
""", unsafe_allow_html=True)

st.markdown(
    '<p style="text-align:center; font-size:14px;">Untuk melihat kod sumber dan projek lengkap, sila lawati: '
    '<a href="https://github.com/username/projek_sijil" target="_blank" style="color:#0b5ed7; text-decoration:underline;">Lihat Projek Sijil Automatik</a>'
    '</p>', 
    unsafe_allow_html=True
)
